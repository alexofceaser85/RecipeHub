...
namespace DesktopClientTests.DesktopClient.Endpoints.Recipes.RecipesEndpointsTests
{
    public class AddRecipeTests
    {
        ...
        public void SuccessfullyAddNewRecipe()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.DoesNotThrow(() => endpoints.AddRecipe("name", "description", false));
                mockHttpMessageHandler
                    .Protected()
        ...
        public void UnsuccessfullyAddNewRecipe()
        {
            ...
            Assert.Multiple(() =>
            {
                var message = Assert.Throws<ArgumentException>(
                    () => endpoints.AddRecipe("name", "description", false))!.Message;
        ...
        public void SessionKeyIsInvalid()
        {
            ...
            Assert.Multiple(() =>
            {
                var message = Assert.Throws<UnauthorizedAccessException>(
                    () => endpoints.AddRecipe("name", "description", true))!.Message;
                Assert.That(message, Is.EqualTo(errorMessage));
